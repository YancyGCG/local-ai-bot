...
const MtlEditorPage = () => {
  ...
  // Get MTL title from the loaded template
  const mtlTitle = (loadedTemplate?.MTL_TITLE as string) || documentDetail?.filename || 'Untitled MTL Document';
...
<div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>      
      ...
      <div style={{ marginBottom: '20px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button
          onClick={handleSave}
          disabled={Boolean(templateError && !hasUserEdited) || saveMutation.isPending || !isValid || !editorValueRef.current}
          style={{
            background: '#10b981',
            color: 'white',
            padding: '10px 20px',
        ...
        <button
          ...
          disabled={Boolean(templateError && !hasUserEdited) || generateMutation.isPending || !isValid || !editorValueRef.current}
          style={{
            background: '#3b82f6',
            color: 'white',
            padding: '10px 20px',
        ...
        >
          {generateMutation.isPending ? 'Generating…' : 'Generate DOCX'}
        </button>
        
        {/* Autosave Settings */}
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginLeft: '20px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#374151', fontSize: '14px', fontWeight: '500' }}>
            <input
              type="checkbox"
          ...
          <label style={{ color: '#374151', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '14px', fontWeight: '500' }}>
            Interval
            <input
              type="number"
              min={1}
              ...
              style={{ width: '50px', padding: '4px 6px', borderRadius: '4px', border: '1px solid #d1d5db', background: 'white', color: '#374151', fontSize: '14px' }}
            />
            seconds
          </label>
                {/* Autosave Status */}
