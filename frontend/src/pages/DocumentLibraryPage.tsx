...
const DocumentLibraryPage = () => {
  ...
  const handleDelete = (docId: string, filename: string) => {
    const confirmed = window.confirm(`Delete "${filename}" and its generated template?`);
    if (!confirmed) {
      return;
    }
...
<div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>      
      <header style={{ 
        ...
        <div>
          ...
          <p style={{ margin: '0 0 0 0', fontSize: '1.1rem', opacity: 0.8 }}>
                ...
                <div style={{ 
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: '500',
                      border: 'none',
                      cursor: deletingId === doc.id && deleteMutation.isPending ? 'not-allowed' : 'pointer',
                      transition: 'background 0.2s'
                    }}
                    onMouseEnter={(e) => {
                      if (!(deletingId === doc.id && deleteMutation.isPending)) {
                        e.currentTarget.style.background = '#dc2626';
                      }
                    }}
                    onMouseLeave={(e) => e.currentTarget.style.background = '#ef4444'}
                  >
                    {deletingId === doc.id && deleteMutation.isPending ? 'Deleting…' : '🗑️ Delete'}
                  </button>
                </div>
              </div>
            ))}
